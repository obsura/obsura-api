# Obsura API Docs

This directory holds the repository documentation that used to live at the
repository root.

## Product

- [REQUIREMENTS.md](product/REQUIREMENTS.md)
- [SCENARIOS.md](product/SCENARIOS.md)
- [NON_GOALS.md](product/NON_GOALS.md)

## Architecture

- [STACK.md](architecture/STACK.md)
- [TOOLING.md](architecture/TOOLING.md)
- [CODE_STANDARDS.md](architecture/CODE_STANDARDS.md)

## Operations

- [CICD.md](operations/CICD.md)

## Community

- [../.github/CODE_OF_CONDUCT.md](../.github/CODE_OF_CONDUCT.md)
- [../.github/CONTRIBUTING.md](../.github/CONTRIBUTING.md)
- [../.github/SECURITY.md](../.github/SECURITY.md)
- [../.github/SUPPORT.md](../.github/SUPPORT.md)
- [../.github/GOVERNANCE.md](../.github/GOVERNANCE.md)

## Legal

- [OPEN_SOURCE_POLICY.md](legal/OPEN_SOURCE_POLICY.md)
- [THIRD_PARTY_NOTICES.md](legal/THIRD_PARTY_NOTICES.md)
- [DEPENDENCY_LICENSE_INVENTORY.md](legal/DEPENDENCY_LICENSE_INVENTORY.md)
